...
namespace Continental.Project.Adam.UI.COM
{
    ...
    /// 
    public class ComModbusTCP
    {
        // ------------------------------------------------------------------------
        #region Variables Config

        public static bool Modbus_UserEnableCom = bool.Parse(System.Configuration.ConfigurationManager.AppSettings["Modbus_EnableCom"].ToString());

        public static string Modbus_ScanIntervalMs = System.Configuration.ConfigurationManager.AppSettings["Modbus_ScanIntervalMs"].ToString();

        public static string Modbus_IpDevice = System.Configuration.ConfigurationManager.AppSettings["Modbus_IpDevice"].ToString();

        public static ushort Modbus_PortCom = ushort.Parse(System.Configuration.ConfigurationManager.AppSettings["Modbus_PortCom"].ToString());
        ...
        public delegate void ExceptionData(ushort id, byte unit, byte function, byte exception, int wrdStartDoubleAddress, string varNames);
        /// <summary>Exception data event. This event is called when the data is incorrect</summary>
        ...
        public static bool NoSyncConnection
        {
        ...
        public ComModbusTCP(string ip, ushort port)
        {
        ...
        public ComModbusTCP(string ip, ushort port, bool no_sync_connection)
        {
        ...
        internal void CallException(ushort id, byte unit, byte function, byte exception, int wrdStartDoubleAddress, string varNames)
        {
            ...
            if (OnException != null) 
                OnException(id, unit, function, exception, wrdStartDoubleAddress, varNames);
        }
        ...
        public void ReadCoils(ushort id, byte unit, ushort startAddress, ushort numInputs)
        {
            if (numInputs > 2000)
            {
                CallException(id, unit, fctReadCoil, excIllegalDataVal, 0, "ReadCoils");
                return;
            }
        ...
        public void ReadDiscreteInputs(ushort id, byte unit, ushort startAddress, ushort numInputs)
        {
            if (numInputs > 2000)
            {
                CallException(id, unit, fctReadDiscreteInputs, excIllegalDataVal, 0, "ReadDiscreteInputs");
                return;
            }
        ...
        public void ReadHoldingRegister(ushort id, byte unit, ushort startAddress, ushort numInputs)
        {
            if (numInputs > 125)
            {
                CallException(id, unit, fctReadHoldingRegister, excIllegalDataVal, 0, "ReadHoldingRegister");
                return;
            }
        ...
        public void ReadInputRegister(ushort id, byte unit, ushort startAddress, ushort numInputs)
        {
            if (numInputs > 125)
            {
                CallException(id, unit, fctReadInputRegister, excIllegalDataVal, 0, "ReadInputRegister");
                return;
            }
        ...
        public void WriteMultipleCoils(ushort id, byte unit, ushort startAddress, ushort numBits, byte[] values)
        {
            ...
            {
                CallException(id, unit, fctWriteMultipleCoils, excIllegalDataVal, startAddress, "WriteMultipleCoils");
                return;
            }
        ...
        public void WriteSingleRegister(ushort id, byte unit, ushort startAddress, byte[] values)
        {
            ...
            {
                CallException(id, unit, fctReadCoil, excIllegalDataVal, startAddress, "WriteSingleRegister");
                return;
            }
            byte[] data;
        ...
        public void WriteSingleRegister(ushort id, byte unit, ushort startAddress, string varName,  byte[] values)
        {
            if (values.GetUpperBound(0) != 1)
            {
                CallException(id, unit, fctReadCoil, excIllegalDataVal, startAddress, "WriteSingleRegister");
        ...
        public void WriteMultipleRegister(ushort id, byte unit, ushort startAddress, byte[] values)
        {
            ...
            if (numBytes > 250)
            {
                CallException(id, unit, fctWriteMultipleRegister, excIllegalDataVal, startAddress, "WriteMultipleRegister");
                return;
            }
        ...
        public void ReadWriteMultipleRegister(ushort id, byte unit, ushort startReadAddress, ushort numInputs, ushort startWriteAddress, byte[] values)
        {
            ...
            if (numBytes > 250)
            {
                CallException(id, unit, fctReadWriteMultipleRegister, excIllegalDataVal, startWriteAddress, "ReadWriteMultipleRegister");
                return;
            }
        ...
        private void WriteAsyncData(byte[] write_data, ushort id)
        {
            if ((tcpAsyCl != null) && (tcpAsyCl.Connected))
            {
                ...
                {
                    CallException(id, write_data[6], write_data[7], excExceptionConnectionLost, 0, "WriteAsyncData");
                }
            }
            else CallException(id, write_data[6], write_data[7], excExceptionConnectionLost, 0, "WriteAsyncData");
        }
        ...
        private void OnSend(System.IAsyncResult result)
        {
            ...
            if (result.IsCompleted == false)
                CallException(0xFFFF, 0xFF, 0xFF, excSendFailt, 0, "OnSend");
            else 
        ...
        private void OnReceive(System.IAsyncResult result)
        {
            ...
            try
            {
                ...
                if (result.IsCompleted == false)
                    CallException(0xFF, 0xFF, 0xFF, excExceptionConnectionLost, 0, "OnReceive");
            }
            catch (Exception) { }
            ...
            if (function > excExceptionOffset)
            {
                ...
                //CallException(id, unit, function, tcpAsyClBuffer[8], 0, "OnReceive");
            }
            // ------------------------------------------------------------
        ...
        private byte[] WriteSyncData(byte[] write_data, ushort id)
        {
            ...
            if (tcpSynCl.Connected)
            {
                try
                {
                    ...
                    if (result == 0)
                        CallException(id, unit, write_data[7], excExceptionConnectionLost, 0, "WriteSyncData");
                    ...
                    {
                        function -= excExceptionOffset;
                        CallException(id, unit, function, tcpSynClBuffer[8], 0, "WriteSyncData");
                        return null;
                    }
                    // ------------------------------------------------------------
                    // Write response data
                ...
                {
                    CallException(id, write_data[6], write_data[7], excExceptionConnectionLost, 0, "WriteSyncData");
                }
            }
            else
                CallException(id, write_data[6], write_data[7], excExceptionConnectionLost, 0, "WriteSyncData");
